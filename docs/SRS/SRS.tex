...
\textcolor{red}{ Items with "High" priority will be addressed first, with subsequent focus shifting to "Medium" and "Low" priorities as the project progresses. This approach ensures that resources and efforts are allocated according to the importance and impact of each milestone on the project's overall success.}
...
\begin{tikzpicture}
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=13cm, height=5cm]{FSM.png}};
    \draw[black, line width=2pt] (image.south west) -- (image.north east);
    \draw[black, line width=2pt] (image.north west) -- (image.south east);
    \end{tikzpicture}

\begin{figure}[ht]
\centering
\includegraphics[width=15cm, height=7cm]{FSM_Updated.png}
\caption{\textcolor{red}{Updated Finite State Machine Diagram}}
\label{fig:fsm_updated}
\end{figure}
