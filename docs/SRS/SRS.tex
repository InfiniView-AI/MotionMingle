...
\begin{description}
    \item[Bandwith] The Application must not occupy more than 20MB/s download speed[1] and 5 MB/s upload speed[2], as these two numbers are the most common home internet speeds for Canadian families.
    \item[Compatibility] The product must be able to operate on all Windows, Mac OS and Linux platforms because over 95\% of the computers run on these three systems[3].
    \item[Security] Data leak in any video conference system is not permitted, according to the Personal Information Protection and Electronic Documents Act [4].
    \item[Ease of Learning] One of the target audiences of our system is senior people. This user group usually has some difficulties in using software as they have lower technical device adoption[7]. The UI interaction must be clear and intuitive enough for senior people to learn all the features without further instructions.
    \item[Compliance] The application is required to comply with all applicable laws[4][5][6] and regulations.
\end{description}
...
\begin{itemize}
    \item[PR4] The system shall have zero fault-tolerant. \label{PR4}
    \begin{itemize}
        \item Rationale: The users must know when they make errors.
        \item Fit Criteria: The system displays warning messages when there are errors.
        \item Priority: Medium
    \end{itemize}
\end{itemize}
...
\begin{table}[h!]
    ...
    HS2 & X & X & & & & &   \\ \hline
    \end{tabular}
    \caption{Traceability Matrix Showing the Connections Between Functional Requirements and Non-functional Requirements} \label{TblTrace}
\end{table}
